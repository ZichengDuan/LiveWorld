...
使用SAM3 (Segment Anything 3) 对视频进行分割
根据文字prompt (如 "person", "car") 分割对应的前景物体
输出每帧的binary mask
"""
...
class Sam3VideoSegmenter:
    ...
    根据文字prompt分割视频中的物体，支持:
    - 单prompt逐个分割后合并
    - 多prompt一次性分割 (multi_prompt=True)
    - 双向传播 (forward/backward/both)
    """
    ...
    def segment_per_category(
        ...
        Returns:
            dict[str, np.ndarray]: 每个prompt对应的mask，形状为 (T, H, W)
        """
    ...
    def segment_instances(
        ...
        """
        Segment instances with SAM3 video predictor.

        Each prompt is processed separately because SAM3's add_prompt resets state
        for text prompts. The detected instances from all prompts are collected
        and assigned unique obj_ids.

        Returns:
            dict[int, np.ndarray]: obj_id -> masks of shape (T, H, W)
        """
        prompts = self.normalize_prompts(prompts)
        if not prompts:
            return {}
        ...
        if frames is None and self.resize_input_to:
        ...
        # Collect masks from all prompts, assigning unique global obj_ids.
        # SAM3's add_prompt resets state for text prompts, so we must process
        # each prompt separately with its own session.
        masks_by_obj: dict[int, dict[int, np.ndarray]] = {}
        next_global_obj_id = 1

        for prompt in prompts:
            # Start a new session for each prompt
            response = self.predictor.handle_request(
                request=dict(
                    type="start_session",
                    resource_path=video_resource,
            ...
            try:
                ...
                # Map SAM3's local obj_ids to our global obj_ids for this prompt
                ...
                for response in self.predictor.handle_stream_request(
                    ...
                    for i, local_obj_id in enumerate(obj_ids.tolist()):
                        mask = masks[i]
                        if mask is None or not mask.any():
                            continue

                        # Assign global obj_id for this local obj_id
                        if local_obj_id not in local_to_global:
                            local_to_global[local_obj_id] = next_global_obj_id
                            next_global_obj_id += 1

                        global_obj_id = local_to_global[local_obj_id]
                        per_obj = masks_by_obj.setdefault(global_obj_id, {})
                        if frame_idx in per_obj:
                            per_obj[frame_idx] = per_obj[frame_idx] | mask
                        else:
                            per_obj[frame_idx] = mask
